...
soia-pkm-clip-github-repo skill 机械层。扫描 `--upstream` 下每个子目录（一个子目录 = 一个
...
两者都可以放进私有 config.yml（见 clip_repo_env.py），不要写进 vault 或本开源 skill 仓库。
"""
...
from clip_repo_env import env_source_hint, load_private_env
