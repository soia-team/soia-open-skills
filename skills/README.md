...
| Skill | Description | Default Prompt |
|---|---|---|
| [`soia-dev-agent-cli-dispatch`](./soia-dev-agent-cli-dispatch/) | Dispatch coding/review/analysis sub-tasks to external coding CLIs (codex, claude, gemini, kimi, opencode, qwen) with explicit executor+model+reasoning control or automatic difficulty-based routing, controlled workdir, An... | Use soia-dev-agent-cli-dispatch: dispatch a coding/review/analysis sub-task to an external coding CLI (codex/claude/gemini/kimi/opencode/qwen), either with an explicit executor+model+reasoning level or letting it auto-route by task difficulty, with controlled workdir, background monitoring, Anti-Fake-Fix verification, and a token/cost summary receipt. |
