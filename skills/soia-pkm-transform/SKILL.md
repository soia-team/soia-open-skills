...
如果目标产物依赖某个 provider（尤其 NotebookLM）但本机缺 CLI / 登录态，不能只报告「没安装」就结束；必须进入 provider bootstrap：先读 [references/providers.md](references/providers.md)，再读具体 provider 文件（NotebookLM 读 [references/provider-notebooklm.md](references/provider-notebooklm.md)，Open Design 读 [references/provider-open-design.md](references/provider-open-design.md)，本地/Obsidian/Codex 读 [references/provider-soia-local.md](references/provider-soia-local.md)）。认证登录必须由用户在 provider 自己的流程里完成，skill 不读取密码、cookie 或私有 token。
...
body: ""
summary: ""
target_output: pdf | ppt | image | long_image | quiz | mindmap | podcast | video | cinematic-video | flashcards | data_table | report | wechat | x_thread | xhs
...
| 播客 / 视频 / 闪卡 / data table | NotebookLM | 不可用时说明需要 provider |
...
NotebookLM provider 的公共实现依赖 `teng-lin/notebooklm-py`；首次使用若本机没有 `notebooklm` CLI，应按 [references/provider-notebooklm.md](references/provider-notebooklm.md) 的 NotebookLM bootstrap 安装、登录、验证，而不是直接降级或停止。`npx -g skills add teng-lin/notebooklm-py` 只安装 agent skill，不安装 Python CLI；不要在项目或 vault 里无 `-g` 地乱装。
...
Open Design 是增强 provider，不是公共 skill 的硬依赖。默认应先用本地 HTML/CSS、当前 agent 文件能力和渲染检查完成；只有用户明确要求、配置指定，或本机已检测到 Open Design 时，才按 [references/provider-open-design.md](references/provider-open-design.md) 的 Open Design bootstrap / handoff 流程使用。
...
- Open Design template / handoff：[references/prompt-open-design.md](references/prompt-open-design.md)
- 视觉报告：[references/prompt-report.md](references/prompt-report.md)
- NotebookLM artifact 路由：[references/prompt-notebooklm.md](references/prompt-notebooklm.md)，再按 artifact 读取 `prompt-notebooklm-ppt.md` / `prompt-notebooklm-image.md` / `prompt-notebooklm-quiz.md` 等；需要全功能 smoke 时读取 [references/notebooklm-test-matrix.md](references/notebooklm-test-matrix.md)
