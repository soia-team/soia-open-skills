...
生成 PPT / 长图 / 封面等视觉产物前，先把最终 prompt 落盘到产物同目录的 `prompts/` 子目录（如 `outputs/transform/<年>/<文章stem>/prompts/NN-<类型>.md`），保证任何一次生成都能回溯、可换 provider 重跑，不依赖记忆里"当时怎么写的"。

位图上出现文字错误（错别字、乱码、排版挤压）时，**唯一合法修复是改 prompt 重新生成**，并保留旧版本供对比；禁止用代码（PIL/Canvas/ImageMagick/OCR 覆盖等）在已生成的图上描字、贴补丁、抠图重绘——那是掩盖问题，不是修复问题。
