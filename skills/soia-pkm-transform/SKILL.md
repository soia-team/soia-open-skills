...
把一篇文章变成不同产物的**公共路由层**：PDF、PPT、图片/长图、试卷、脑图、播客、闪卡、报告、公众号 HTML、小红书卡片等。
...
如果用户没有给文章来源，先问一句要转换哪篇；如果只缺风格偏好，使用通用默认，不阻塞。
...
| 目标 | 默认 provider | 常见降级 |
|------|---------------|----------|
| PDF | Obsidian 原生导出（vault 内 Markdown） | agent PDF 工具 / 浏览器打印 |
| PPT / PPTX | Codex presentations 或 NotebookLM slide-deck | HTML slides / PDF deck |
| 图片 / 长图 / 信息图 | imagegen 或 HTML screenshot | NotebookLM infographic |
...
使用 NotebookLM、Obsidian、Codex 内置文件能力或发布链路前，按需读取 [references/providers.md](references/providers.md)。

## 工作流

1. **解析请求**：识别文章来源、目标产物、格式、受众、是否需要先归档。
2. **获取文章**：URL 走对应 `clip-*`，本地文件直接读取，形成 article packet。
3. **选择 recipe**：按目标产物读取 [references/output-recipes.md](references/output-recipes.md)，选择 provider。
4. **生成产物**：只在当前 vault / 项目 / 用户指定目录写文件。
5. **验证**：
...
6. **回写链接**：如果源文章在 vault 内且配置允许，新增或更新 `## 转化产物` 段，只追加产物链接，不改原文。
7. **回执**：列出来源、provider、输出文件、验证结果和下一步。
...
3. **文件变更**：新建 / 修改文件完整路径。
4. **验证结果**：实际跑过的检查。
5. **下一步**：可选，如「发布这篇」或「把 PPT 再转 PDF」。
