...
两者都可以放进私有 env 文件（见 soia_env.py），不要写进 vault 或本开源 skill 仓库。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--upstream",
        help="上游 git 仓库根目录；不传则读取私有 env 的 SOIA_REPO_UPSTREAM_DIR（二选一，--upstream 优先）",
    )
    parser.add_argument(
        "--vault",
...
def main() -> int:
    ...
    if args.add:
        if not upstream:
            print(
                f"错误：未指定 --upstream 且未设置私有 env 的 SOIA_REPO_UPSTREAM_DIR（{env_source_hint()}）",
                file=sys.stderr,
            )
            return 1
    ...
    if not upstream or not upstream.is_dir():
        print(
            f"错误：--upstream 目录不存在或未指定（也可设置私有 env 的 SOIA_REPO_UPSTREAM_DIR，"
