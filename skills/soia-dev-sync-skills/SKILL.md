...
```bash
python3 skills/soia-dev-sync-skills/scripts/sync_soia_skills.py \
...
```text
~/.config/soia-skills/soia-open-skills/soia-dev/soia-dev-sync-skills/config.yml
SOIA_DEV_SYNC_SKILLS_CONFIG_FILE=<custom-config-path>
```
...
```bash
# 查看内置目标或共享源中的技能
python3 skills/soia-dev-sync-skills/scripts/sync_soia_skills.py --list-targets
python3 skills/soia-dev-sync-skills/scripts/sync_soia_skills.py --source-dir <shared-skill-dir> --list-skills
...
审计日志写到 `${XDG_STATE_HOME:-~/.local/state}/soia-dev-sync-skills/`，最多保留 20 个。它记录参数、链接变更和汇总，不写入密钥内容。
...
```bash
python3 skills/soia-dev-sync-skills/scripts/sync_soia_skills.py --list-targets
python3 -m py_compile skills/soia-dev-sync-skills/scripts/sync_soia_skills.py
```
