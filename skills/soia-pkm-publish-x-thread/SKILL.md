...
description: 把成文草稿改写成 X thread：拆成带 (1/N) 编号的推文串，首条抓钩子、末条给 CTA，每条 ≤280 字符。默认只产文本；可选浏览器直填模式把内容填进已登录 X 撰写框存草稿，明确授权才点发布。不接 X API。Triggers：「发成 X thread」「拆成推文串」「thread 这篇」「发条 X」「发个推」
version: 1.1.0
...
## 浏览器直填模式（可选，需已登录 X 的浏览器面）

客户说「填进 X」「存到 X 草稿」「直接发」时启用；无浏览器面或未登录则回退纯文本输出并说明。不需要任何 Premium 订阅。

**流程**：优先 claude-in-chrome（复用真实 Chrome 登录态），导航 `https://x.com/compose/post` → 逐字输入首条 → thread 多条时用「+」按钮逐条追加 → 按授权档位收尾。

**两档授权，绝不越档**：

| 档位 | 触发条件 | 收尾动作 |
|---|---|---|
| 存草稿（默认） | 客户只说「填进去/存草稿」 | 点关闭「×」→ 弹层中选「保存」进未发送帖子；回执告知草稿位置（更多 > 未发送帖子） |
| 直接发布 | 客户**在本次会话中逐字给出内容或确认最终文案，并明确说「发」** | 点「发帖」，发布后到个人主页取推文 URL 写进回执 |

红线：默认档绝不点「发帖」；内容是 AI 改写产物（非客户逐字给出）时，即使客户此前说过「直接发」，也必须先把最终文案展示给客户确认后才可发布；登录态只留在浏览器，不导出 cookie；发布档必须逐字使用客户确认过的文案，不得代改。

**回执**：草稿档报「已存入未发送帖子 + 条数」；发布档报推文 URL；中途失败（撰写框改版/登录失效）如实报告停在哪一步。
