...
> **覆盖状态：`partial_coverage`**。Catalog 价格资料列出 10 个 OpenAI 型号和 10 个 Anthropic
> 实际型号，本文件只记录其中 6 个 Codex 型号和 3 个 Claude 型号；且原始 manifest 未随交接提供。
> 因此本文只证明已覆盖组合的聚合结果，不证明全 catalog 矩阵已经闭环。
...
- `references/model-catalog.yml`：`gpt-5.6-sol`/`gpt-5.6-terra`/`gpt-5.6-luna`/`claude-sonnet-5`/
  `claude-opus-4-8`/`claude-haiku-4-5` 的 `routing_profile` /
  ...
  `[low, medium, high, xhigh, max]`。Sonnet 5 的 2026-09-01 标准价作为同一 model ID 内的
  `future_pricing` 保存，不再伪装成第二个可调用模型。未出现在本文件矩阵里的模型（gemini/kimi/opencode/qwen 全家族、
