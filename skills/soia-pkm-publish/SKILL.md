...
- **放哪**：秘钥只放私有 env 文件，不进 vault、不进这个开源 skill 仓库、也不进 shell 启动文件。`publish.py` 已接入 `scripts/soia_env.py`，跑起来会**自动探测并加载** `$SOIA_PKM_ENV_FILE` / `~/.config/soia-pkm/env` / `~/.soia-pkm.env`（与 `soia-pkm-library` / `soia-pkm-clip-x` / `soia-pkm-maintain` 同一套约定），把凭据写进这些文件即可，**无需手动 source**：

  ```
  # ~/.config/soia-pkm/env（或任意你惯用的私有 env 路径）
  WECHAT_APP_ID=wx************
  WECHAT_APP_SECRET=********************************
  ```
  ...
  （凭据放在非默认路径时：`SOIA_PKM_ENV_FILE=/你的/路径 python3 scripts/publish.py ...`。）
