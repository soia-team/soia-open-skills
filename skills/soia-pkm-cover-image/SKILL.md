...
PKM 发布链（`compose -> publish-*`）里的**封面生成环节**：给已经写好或即将发布的文章生成一张位图封面，公众号场景的产出可直接喂给 `soia-pkm-publish-wechat-draft` 的 `--cover` 参数。本技能只产出**一张封面位图**（文中配图/插画属于 v2 路线图，见文末），不做排版、不做发布、不碰账号凭据。
...
4. 生成前把最终 prompt 落盘到 `prompts/NN-cover-<slug>.md`；公众号场景生成后的 `cover.png` 可直接作为下游 `soia-pkm-publish-wechat-draft` 的 `scripts/publish.py --article <md> --cover <output-dir>/cover.png` 输入（`soia-pkm-publish-wechat-draft` 是独立的下游 skill，按其自身安装说明配置，本技能不修改它）。
...
| `soia-pkm-publish-wechat-draft` | 下游衔接（非本技能依赖） | 见该 skill 自身的安装说明 | 本技能只负责产出 `cover.png`；是否安装、何时调用公众号草稿箱 skill 由客户决定，本技能不代为安装或修改它 |
...
这条命令属于 `soia-pkm-publish-wechat-draft` 自己的脚本，客户需先按该 skill 的安装说明配置好凭据；本技能只负责把 `cover.png` 生成到约定路径，不代替公众号草稿箱 skill 执行发布。
