...
from check_env import check_environment, resolve_binary
...
def execute_plan(plan: dict[str, Any], *, binary: str, timeout: int) -> dict[str, Any]:
    ...
    success = all(step["returncode"] == 0 for step in steps.values()) and output.is_file()
    return {"success": success, "output": str(output), "steps": steps}
