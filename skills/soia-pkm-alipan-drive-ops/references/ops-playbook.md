...
从 2.2.0 起，扫描器对同名兄弟目录 fail-closed：每个目录行都增加 `"ambiguous_name": true`，全部不下钻，并在 `.errors` sidecar 为每个对象记录一条 `AMBIGUOUS_NAME`。消费端看到非空 errors 必须停止信任本次结构审计，不能把这些目录判为空。消歧时先临时 rename 每个同名目录使路径唯一，再以唯一路径定向补扫对应子树，核对 file_id、文件数和容量后改回原名；若改回后仍需长期可扫描，应保留唯一命名或接受该节点持续 fail-closed。
