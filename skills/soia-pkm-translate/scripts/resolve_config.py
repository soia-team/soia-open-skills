#!/usr/bin/env python3
"""Locate the optional soia-pkm-translate config without hardcoded user paths.
...
OVERRIDE_CONFIG_NAME = "SOIA_PKM_TRANSLATE_CONFIG_FILE"
# Older spelling accepted as a compatibility alias; new docs should prefer
# SOIA_PKM_TRANSLATE_CONFIG_FILE.
OVERRIDE_ENV_NAME = "SOIA_PKM_TRANSLATE_ENV_FILE"
CONFIG_NAMES = ("config.yml", "config.yaml", "config.json")
DEFAULT_CONFIG_DIR = "~/.config/soia-skills/soia-open-skills/soia-pkm/soia-pkm-translate"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Locate the optional soia-pkm-translate config.")
    ...
    if args.json:
        ...
        if found:
            ...
            print("No soia-pkm-translate config found; using built-in defaults (target_language=zh-CN, mode=normal).")
