...
默认策略：单画布流程图选 `VISIO文件 beta`，多画布选 `导出全部画布 (.vsdx)`；思维导图选 `Xmind文件`。两类都建议同时保留 POS/POSF/POSM。若默认格式缺失、会员/权限阻断或内容校验失败，回退 POS 并报告原因。无法确认图表类型时加入待人工确认队列，由客户和 Agent 一起确认后再下载，不依据颜色或单一图标盲猜。
