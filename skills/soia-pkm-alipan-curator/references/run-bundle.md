...
批量执行器默认限制同一 drive 最多 2 个并发进程，并在 XDG state 中使用跨 workspace 的进程锁；第 3 路会在任何云盘写入前失败。不要为了提速复制脚本或改变状态目录绕过限制。若历史执行曾在高并发下留下 `failed`，先用 `ll` 证明源仍在、目标不存在，再沿用原计划和原账本 `--resume`，让最新终态覆盖历史失败结论。
