...
历史 `mkdir` 的 ledger 若缺少 `file_id`，只能由 `run.json.files` 中标签以 `directory_identities` 开头的、已登记且存在的 JSONL 旁证恢复。每条有效旁证必须有非空 `action_id`、规范化绝对 `path` 和非空目录 `id`；preflight 只接受同时精确匹配 ledger/plan 的 `action_id + path`，绝不作 path-only、前缀或模糊匹配。文件缺失、条目缺失、冲突、`status: "missing"` 或 action/path 不匹配都会阻断预检。该输入会与 manifest、计划和其他不可变输入一起写入预检报告的 SHA-256 门禁，不能以未登记的临时文件替代。
