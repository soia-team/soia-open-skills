...
description: 阿里云盘资源顾问，在 soia-pkm-alipan 原子操作上提供 inventory/organize/catalog/plan 四类工作流：盘点云盘、整理资源、生成 Obsidian 馆藏与分区缓存式增量 Excel 总索引、基于本次用户提供的学情生成学习计划。Triggers：「整理云盘」「云盘盘点」「更新云盘索引」「更新Excel总索引」「给云盘建图书馆」「用网盘资源做学习计划」
---
...
阿里云盘资源顾问，在 soia-pkm-alipan 原子操作上提供 inventory/organize/catalog/plan 四类工作流：盘点云盘、整理资源、生成 Obsidian 馆藏与增量 Excel 总索引、基于本次用户提供的学情生成学习计划
...
**全区深度重组（盘点→方案→裁定→分批→索引→消费端联动 六步全流程）→ 必读 [references/deep-reorg-playbook.md](references/deep-reorg-playbook.md)**（2026-07 英语启蒙实战提炼：SHA1 删除证据、同名不同哈希隔离不删、消费端 file_id 红线、多 AI 协作边界、完成定义）。
...
需要可筛选 Excel 时，调用正式入口 **`scripts/gen_catalog_xlsx.py`**，不要在 vault、临时目录或会话产物目录另写一次性 builder。完整参数、依赖与输出结构见 [references/catalog-excel.md](references/catalog-excel.md)。核心约定：

- 输出为轻量 `00_阿里云盘馆藏总索引.xlsx` + 相邻的 6 个分区明细工作簿；总入口保留目录、类型、扩展名、分区统计和直达链接。
- 默认按每份 `_全文检索/*.md` 的 SHA-256 增量刷新；哪个分区变化只重建哪个，未变化分区不改写。总览或任一分区变化时只额外重建轻量总入口。
- 首次生成或生成规则整体升级才使用 `--force`；日常刷新禁止无理由全量重建。
- 解析缓存只进 `${XDG_CACHE_HOME:-~/.cache}/soia-pkm-alipan-curator/catalog-xlsx/`，不进 vault/仓库。
- Excel 作者层必须使用宿主提供的 `@oai/artifact-tool`；缺失时停止并说明，不临时换库。交付验收时加 `--verify`，有 `soffice` 时传入以预计算链接显示值。
- 分区需要面向家长的简明说明、筛选表和课程直达链接时，调用 `scripts/gen_family_nav_xlsx.mjs`；输入为结构化 JSON，输出两张表的家庭导航 Excel。传 `--soffice` 预计算 `HYPERLINK` 显示值，避免在线预览只显示公式。

**图书馆建法（浏览/检索/策展 + 分类方案）**（精选资源沉淀为可查询馆藏时用，模板与字段详见 `references/library-method.md`）：
1. **00_馆藏总览**：全盘索引 MOC。用 `scripts/gen_catalog.py --scan-dir <扫描JSONL目录> --out <总览.md> [--moves f --deletes f --roots f --max-heading-depth N]` 生成——**只有 `NN_` / `NN.` 编号业务目录成为标题，标题直接显示当前实体目录名（如 `10_SuperSimpleSongs`）且课程名本身可点击直达网盘，不拼冗长累计编号链；无编号的内部素材目录不进入大纲，若其下没有新的编号业务目录，则在最近编号父目录聚合成一行，不展开 `images/Scripts/banner` 等技术路径**。同一编号目录下超过 12 个需要单列的素材文件夹压缩为「前2+计数+末1」；标题最多到 H6，或按 `--max-heading-depth` 提前封顶，深层编号目录仍保持 Markdown 标题，不输出 HTML 缩进实体。总览本身不铺单个文件保持清爽。**别为做"全盘可筛选数据库"给每个文件夹建桩笔记**：Bases 只能查独立笔记→几千个机器桩、与总览重复、撑翻库，得不偿失（已验证放弃）；导航靠总览全文搜足够。**要搜到单个文件**：加 `--search-dir <目录> --junk <模板碎片前缀>`，按区各出一份「全文检索」文件（每文件一行路径+大小，只搜不看）——浏览(总览)/检索(_全文检索)/策展(精选卡)三样分开，都由本脚本一条命令产出。
