...
1. **00_馆藏总览**：全盘索引 MOC。用 `scripts/gen_catalog.py --scan-dir <扫描JSONL目录> --out <总览.md> [--moves f --deletes f --roots f]` 生成——**折叠标题（分类夹 NN_ 前缀→标题）+ 资源级表格（第一个非 NN_ 目录→一行，带 🔗直达/媒介/文件数/大小）**，点 🔗 到网盘看文件、总览本身不铺文件保持清爽（几万文件全铺=不可读，实测 41k 行→600 行）。**别为做"全盘可筛选数据库"给每个文件夹建桩笔记**：Bases 只能查独立笔记→几千个机器桩、与总览重复、撑翻库，得不偿失（已验证放弃）；导航靠总览全文搜足够。
