...
PKM 闭环的**公众号发布环节（发）**：把 `compose` 产出的成文草稿排版、校验，并在确认后推入微信公众号草稿箱。
...
```bash
npx skills add soia-team/soia-open-skills -g -a '*' -s soia-pkm-publish-wechat-draft -y
```

配置约定：

```text
~/.config/soia-skills/soia-open-skills/soia-pkm/soia-pkm-publish-wechat-draft/config.yml
SOIA_PKM_PUBLISH_WECHAT_DRAFT_CONFIG_FILE=<custom-config-path>
```
...
- **放哪**：秘钥只放私有 `config.yml`，不进 vault、不进这个开源 skill 仓库、也不进 shell 启动文件。`publish.py` 已接入 `scripts/publish_env.py`，跑起来会**自动探测并加载** `$SOIA_PKM_PUBLISH_WECHAT_DRAFT_CONFIG_FILE`（或兼容别名 `$SOIA_PKM_PUBLISH_ENV_FILE`）以及默认路径 `~/.config/soia-skills/soia-open-skills/soia-pkm/soia-pkm-publish-wechat-draft/config.yml`，**无需手动 source**：
  ...
  （凭据放在非默认路径时：`SOIA_PKM_PUBLISH_WECHAT_DRAFT_CONFIG_FILE=/你的/路径/config.yml python3 scripts/publish.py ...`。）
