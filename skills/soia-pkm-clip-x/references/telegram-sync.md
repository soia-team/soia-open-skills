...
- 把 `TELEGRAM_API_ID` / `TELEGRAM_API_HASH` / `TELEGRAM_SESSION_STRING` 放进私有 env 文件（`$SOIA_PKM_ENV_FILE`、`~/.config/soia-pkm/env` 或 `~/.soia-pkm.env`），不要放进 vault 或提交到开源 skill 仓库
