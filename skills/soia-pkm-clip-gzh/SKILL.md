...
```bash
npx skills add soia-team/soia-open-skills -g -a '*' -s soia-pkm-clip-gzh -y
```

配置约定：

```text
~/.config/soia-skills/soia-open-skills/soia-pkm/soia-pkm-clip-gzh/config.yml
SOIA_PKM_CLIP_GZH_CONFIG_FILE=<custom-config-path>
```
...
- 客户同意后，把 `{accepted: true, acceptedAt: <ISO 时间戳>, disclaimerVersion: "v1"}` 写入私有配置目录下的 `consent.json`（与 `config.yml` 同级：`~/.config/soia-skills/soia-open-skills/soia-pkm/soia-pkm-clip-gzh/consent.json`）。
