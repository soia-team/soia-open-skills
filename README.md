...
```
 ...
 └─────────┘  └─────────┘  └─────────┘  └─────────┘  └─────────┘
  ...
  支撑：soia-pkm-bootstrap（一句话从零搭 vault + 接入多 AI）
        soia-pkm-transform-article-pdf/slides/visual/notebook（转化线：按输出类型拆分）
...
| skill | 说明 | 现在能用? | 依赖 |
|-------|------|----------|------|
| [`soia-pkm-transform-article-pdf`](./skills/soia-pkm-transform-article-pdf/) | **文章 → PDF**：Obsidian 原生导出优先，vault 外降级 pandoc/weasyprint | ✅ 可用 | Obsidian（vault 内）或 pandoc/weasyprint（降级）|
...
| [`soia-pkm-transform-article-notebook`](./skills/soia-pkm-transform-article-notebook/) | **文章 → 试卷/闪卡/脑图/播客**：NotebookLM 优先，降级本地 Markdown | ✅ 可用 | NotebookLM 可选；降级无额外依赖 |
