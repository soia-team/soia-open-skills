...
- 可把已批准的 OpenClaw / TweetClaw JSON 或 JSONL 导出作为补充上下文
...
如果用户已经有 OpenClaw / TweetClaw 导出的 JSON 或 JSONL 文件，可以把它放在
fxtwitter 结果旁边当作只读补充材料。agent 应该保留原始 URL、tweet ID、作者、
抓取时间、截图路径和 metrics 来源，只用这些信息补全 frontmatter、Summary、
topics 和 Related，不覆盖用户写的 `## My Thoughts`。

不要把导出文件当成发布授权。归档、摘要和双链整理是安全范围；点赞、回复、
转发、关注、私信和发推都必须另行取得用户明确确认。
