...
import officecli_inspect  # noqa: E402
import officecli_env  # noqa: E402
...
class CheckEnvironmentTests(unittest.TestCase):
    def test_missing_binary_fails_closed(self) -> None:
        with mock.patch.object(officecli_env.shutil, "which", return_value=None):
            result = officecli_env.check_environment({})
    ...
    def test_compatible_version_is_accepted(self) -> None:
        ...
        with mock.patch.object(officecli_env, "resolve_binary", return_value="/tool/officecli"), mock.patch.object(
            officecli_env.subprocess, "run", return_value=completed
        ):
            result = officecli_env.check_environment({})
    ...
    def test_executes_copy_mutation_and_validation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            completed = [
                ...
                mock.Mock(returncode=0, stdout="{}", stderr=""),
                mock.Mock(returncode=0, stdout='{"success": true}', stderr=""),
                mock.Mock(
                    returncode=0,
                    stdout='{"success": true, "data": {"count": 2, "issues": []}}',
                    stderr="",
                ),
            ]
            with mock.patch.object(officecli_safe.subprocess, "run", side_effect=completed) as run:
                result = officecli_safe.execute_plan(plan, binary="/tool/officecli", timeout=30)

            self.assertTrue(result["success"])
            self.assertTrue(result["schema_clean"])
...
class SafeInspectionTests(unittest.TestCase):
    def test_rejects_non_read_commands(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            source = Path(directory) / "source.docx"
            source.write_bytes(b"source")
            with self.assertRaisesRegex(ValueError, "read allowlist"):
                officecli_inspect.build_plan(
                    input_path=source,
                    office_args=["raw-set"],
    ...
    def test_inspection_uses_temporary_copy(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            source = Path(directory) / "source.docx"
            source.write_bytes(b"original")
            plan = officecli_inspect.build_plan(
                input_path=source,
                office_args=["view", "issues", "--json"],
                artifact_output=None,
                overwrite=False,
            )
            completed = mock.Mock(returncode=0, stdout='{"success": true}', stderr="")
            with mock.patch.object(officecli_inspect.subprocess, "run", return_value=completed) as run:
                result = officecli_inspect.execute_plan(plan, binary="/tool/officecli", timeout=30)

            self.assertTrue(result["success"])
            self.assertTrue(result["source_unchanged"])
            self.assertEqual(len(result["source_sha256"]), 64)
            self.assertEqual(source.read_bytes(), b"original")
            inspected_path = Path(run.call_args_list[0].args[0][2])
            self.assertNotEqual(inspected_path, source)
            self.assertEqual(run.call_args_list[0].args[0][1], "view")
